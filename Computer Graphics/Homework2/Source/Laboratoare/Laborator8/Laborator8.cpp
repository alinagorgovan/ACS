...
{
	{
		...
		RenderSimpleMesh(meshes["sphere"], shaders["ShaderLab8"], modelMatrix, glm::vec3(0.8, 0.2, 0.1));
	}
...

	{
		...
		RenderSimpleMesh(meshes["box"], shaders["ShaderLab8"], modelMatrix, glm::vec3(0.5, 0, 0.5));
	}
	...
	// Render ground
	{
		...
		RenderSimpleMesh(meshes["plane"], shaders["ShaderLab8"], modelMatrix, glm::vec3(0.1, 0.5, 0.1));
	}
...
void Laborator8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
	...
	int spotOn = glGetUniformLocation(shader->program, "spot");
	glUniform1i(spotOn, spot);
...
void Laborator8::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
	{
		...
		if (window->KeyHold(GLFW_KEY_R)) angle += deltaTime;
		if (window->KeyHold(GLFW_KEY_T)) angle -= deltaTime;

		if (window->KeyHold(GLFW_KEY_O) && lightDirection.y < 1) lightDirection.y += deltaTime * 2;
		if (window->KeyHold(GLFW_KEY_P) && lightDirection.y > -1) lightDirection.y -= deltaTime * 2;
		if (window->KeyHold(GLFW_KEY_L) && lightDirection.x < 1) lightDirection.x += deltaTime * 2;
		if (window->KeyHold(GLFW_KEY_J) && lightDirection.x > -1) lightDirection.x -= deltaTime * 2;
		if (window->KeyHold(GLFW_KEY_I) && lightDirection.z < 1) lightDirection.z += deltaTime * 2;
		if (window->KeyHold(GLFW_KEY_K) && lightDirection.z > -1) lightDirection.z -= deltaTime * 2;
	}
