...
void Laborator2::Init()
{
	...
	// Create a mesh box using custom data
	{
		...
		{
			VertexFormat(glm::vec3(0, 0,  1), glm::vec3(139.0/255, 40.0/255, 41.0/255)),
			VertexFormat(glm::vec3(1, 0,  1), glm::vec3(139.0 / 255, 40.0 / 255, 41.0 / 255)),
			VertexFormat(glm::vec3(0, 1,  1), glm::vec3(139.0 / 255, 40.0 / 255, 41.0 / 255)),
			VertexFormat(glm::vec3(1, 1,  1), glm::vec3(139.0 / 255, 40.0 / 255, 41.0 / 255)),
			VertexFormat(glm::vec3(0, 0,  0), glm::vec3(139.0 / 255, 40.0 / 255, 41.0 / 255)),
			VertexFormat(glm::vec3(1, 0,  0), glm::vec3(139.0 / 255, 40.0 / 255, 41.0 / 255)),
			VertexFormat(glm::vec3(0, 1,  0), glm::vec3(139.0 / 255, 40.0 / 255, 41.0 / 255)),
			VertexFormat(glm::vec3(1, 1,  0), glm::vec3(139.0 / 255, 40.0 / 255, 41.0 / 255)),
		...
		{
			VertexFormat(glm::vec3(0.25f, 0,  1), glm::vec3(139.0 / 255, 40.0 / 255, 41.0 / 255)),
			VertexFormat(glm::vec3(1, 0,  0.25f), glm::vec3(1, 0, 0)),
			VertexFormat(glm::vec3(0, 0,  0), glm::vec3(0, 1, 0)),
			VertexFormat(glm::vec3(0.5f, 1,  0.5f), glm::vec3(0, 0.5, 1)),
			// TODO: Complete the information for the cube
		};
		...
		vector<VertexFormat> vertices3
		{
			...
			VertexFormat(glm::vec3(0, 0,  0), glm::vec3(0, 0.5, 1)),
			// TODO: Complete the information for the cube
		};
		...
		vector<unsigned short> indices3 =
		{
			...
			0, 2, 1	// indices for second triangle
		...
		vertices_circle.push_back(VertexFormat(glm::vec3(0, 0, 0), glm::vec3(1, 0, 0)));
		vertices_circle.push_back(VertexFormat(glm::vec3(r, 0, 0), glm::vec3(1, 0, 0)));

		for (int i = 1; i < n; i++) {
			vertices_circle.push_back(VertexFormat(glm::vec3(r * cos(alpha), r * sin(alpha), 0), glm::vec3(1, 0, 0)));
			alpha += n / (2 * M_PI);
		}
		for (int i = 1; i < n; ++i) {
...
void Laborator2::Update(float deltaTimeSeconds)
{
	...
	RenderMesh(meshes["cube3"], shaders["VertexColor"], glm::vec3(1.5f, 0, -1), glm::vec3(0.25f));

	RenderMesh(meshes["pyramid"], shaders["VertexColor"], glm::vec3(1.5f, 0, 0.5f));

	RenderMesh(meshes["square"], shaders["VertexColor"], glm::vec3(0.5f, 0, 0));

	RenderMesh(meshes["circle"], shaders["VertexColor"], glm::vec3(1, 1, 0));
