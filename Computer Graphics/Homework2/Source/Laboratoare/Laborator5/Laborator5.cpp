...
void Laborator5::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
	{
		...
		if (window->KeyHold(GLFW_KEY_O)) {
			projectionMatrix = glm::ortho(xleft, xright, xbottom, xtop, 0.01f, 200.0f);
		}

		if (window->KeyHold(GLFW_KEY_P)) {
			// TODO : translate the camera up
			projectionMatrix = glm::perspective(RADIANS(xfov), window->props.aspectRatio, 0.01f, 200.0f);
		}
		if (window->KeyHold(GLFW_KEY_M)) {
			xfov += deltaTime * cameraSpeed * 5;
			projectionMatrix = glm::perspective(RADIANS(xfov), window->props.aspectRatio, 0.01f, 200.0f);
		}
